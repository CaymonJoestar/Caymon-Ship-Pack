...
public class csp_newmechEffect extends BaseCombatLayeredRenderingPlugin implements EveryFrameWeaponEffectPlugin {

    private boolean runOnce = false;
    private ShipAPI ship;

    private WeaponAPI head, armL, armR, pauldronL, pauldronR, gun, shoulderwep, headGlow;
    ...
    private float overlap = 0, heat = 0, originalArmPos = 0f, originalShoulderPos = 0f, originalExtraPos = 0f;
            ...
            switch (w.getSlot().getId()) {
                case "AGUN":
                    if (gun == null) {
                        ...
                        if (w.getBarrelSpriteAPI() != null) {
                            w.getBarrelSpriteAPI().getCenterY();
                        }
                        limbInit++;
                    }
                    break;
                case "AGUN2":
                    if (armL == null) {
                        ...
                        if(armL.getMissileRenderData() != null)
                            originalExtraPos = armL.getMissileRenderData().get(0).getSprite().getCenterY();
                        limbInit++;
                    }
                    break;
                case "C_ARMR":
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if (armL != null) 
        {
            if(weapon.isFiring() && !windingUp)
            {
                baseAngle = armL.getCurrAngle();
                windingUp = true;
            }
            if (!noanim) {
                ...
                else if(armL.getSpec().getWeaponId().equals("armaa_ashura_leftArm"))
                    sineC = MagicAnim.smoothNormalizeRange(ship.getSystem().getEffectLevel(), 0.0f, 1f);
                ...
                if(armL.getMissileRenderData() != null)
                    armL.getMissileRenderData().get(0).getSprite().setCenterY(originalExtraPos - (16 * sinceB) + (8 * sinceG));
            }
        }
        ...
        if (weapon != null) {
            if (armL != null) {
                if (!noanim) 
                {
                    ...
                    float animOffset =
                        (sineA * (TORSO_OFFSET / 7f) * 0.7f)
                      - (sinceG * TORSO_OFFSET * 0.5f)
                      + (sineC * TORSO_OFFSET);
                    if(windingUp)
                        armL.setCurrAngle(baseAngle + animOffset);
                } else {
                ...
                if(armL.getChargeLevel() >= 0 && windingUp)
                {
                    windingUp = false;
                    baseAngle = weapon.getCurrAngle();
                }
            }
        }
